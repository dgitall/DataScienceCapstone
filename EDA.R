...
d <- data.frame(word = NULL,freq=NULL)
...
wordsTotal <- sum(d$freq)
d <- d %>% arrange(desc(freq)) 
d<- d %>% mutate(fract = freq/wordsTotal)
cumfract = cumsum(d$fract)
d <- cbind(d, as.data.frame(cumfract))

# Get the list of words that make up half of the total occurrence of words
dhalf <- filter(d, cumfract <= 0.50)
# Plot a distribution of all words and of the most common
smD <- d[which(d$freq>500),]
...
MeanX <- mean(d$freq,na.rm=TRUE)
MedX <- median(d$freq, na.rm=TRUE)
Max <- max(d$freq,na.rm=TRUE)
MaxY <- round(which.max(density(d$fract,na.rm=TRUE)$y), 0)
MaxX <- round(density(d$fract,na.rm=TRUE)$x[MaxY], 0)
...
#wordcloud
#limit words by specifying min frequency
wordcloud(smBGram$word,smBGram$freq, min.freq=200)
